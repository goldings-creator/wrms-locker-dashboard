        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <form onSubmit={handleAddOrUpdateLocker} className="bg-white p-8 rounded-3xl w-full max-w-md space-y-4 shadow-2xl">
            ...
            <div className="space-y-3">
              <div><label className="text-[10px] font-black uppercase text-slate-400 mb-1 block tracking-widest">Locker #</label><input name="lockerNumber" required defaultValue={editingLocker?.lockerNumber} className="w-full p-3 bg-slate-50 rounded-xl outline-none border border-transparent focus:border-blue-200 font-mono font-bold" /></div>
              <div><label className="text-[10px] font-black uppercase text-slate-400 mb-1 block tracking-widest">Combination</label><input name="combination" required defaultValue={editingLocker?.combination} className="w-full p-3 bg-slate-50 rounded-xl outline-none border border-transparent focus:border-blue-200 font-mono font-bold" /></div>
              <div><label className="text-[10px] font-black uppercase text-slate-400 mb-1 block tracking-widest">Location</label><input name="location" defaultValue={editingLocker?.location} className="w-full p-3 bg-slate-50 rounded-xl outline-none border border-transparent focus:border-blue-200 font-bold" /></div>
            </div>
            <div className="flex gap-2 pt-4">
              ...
              <button type="submit" className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold uppercase text-xs tracking-widest shadow-lg shadow-blue-200">Save Record</button>
            </div>
          </form>
        </div>
      )}
      ...
      {isAssignModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <form onSubmit={handleAssignStudent} className="bg-white p-8 rounded-3xl w-full max-w-md space-y-4 shadow-2xl">
            <div className="bg-emerald-50 w-12 h-12 rounded-xl flex items-center justify-center text-emerald-600 mb-2 shadow-inner"><UserPlus className="w-6 h-6"/></div>
            <h2 className="text-2xl font-black tracking-tight">Assign Locker #{activeLockerForAssign?.lockerNumber}</h2>
            <p className="text-sm text-slate-500 font-medium">Type the full name of the student who will use this locker.</p>
            <input name="studentName" required autoFocus placeholder="Student Full Name" className="w-full p-4 bg-slate-50 rounded-xl outline-none border border-transparent focus:border-emerald-200 font-bold text-lg shadow-sm" />
            <div className="flex gap-2 pt-4">
              ...
              <button type="submit" className="flex-1 py-3 bg-emerald-600 text-white rounded-xl font-bold uppercase text-xs tracking-widest shadow-lg shadow-emerald-200">Assign Now</button>
            </div>
          </form>
        </div>
      )}
