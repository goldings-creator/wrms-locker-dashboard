...
const App = () => {
  ...
  useEffect(() => {
    signInAnonymously(auth).catch(err => notify("Login failed", "error"));
  ...
  const clearDatabase = async () => {
    if (!window.confirm("This will delete everything. Continue?")) return;
    setIsUploading(true);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20">
      <header className="bg-white border-b sticky top-0 z-40 p-4 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-4">
          <div className="bg-blue-600 p-2 rounded-lg text-white font-black text-sm shadow-md">WRMS</div>
        ...
        <div className="flex items-center gap-2">
          <div className="hidden lg:flex gap-1 items-center mr-4 border-r pr-4">
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8">
        ...
        <div className="relative mb-10">
          ...
          <input type="text" placeholder="Search Number or Student..." className="w-full pl-14 pr-6 py-5 bg-white border border-slate-200 rounded-[1.5rem] outline-none focus:ring-4 focus:ring-blue-50 transition-all shadow-sm text-lg font-medium" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
...
// --- RENDER LOGIC (THE "SPARK" TO SHOW THE APP) ---
